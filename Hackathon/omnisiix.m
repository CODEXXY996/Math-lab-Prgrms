...
% Use only a segment of the audio for processing
segment_length = 10; % Length of segment in seconds
num_samples = min(segment_length * fs, length(audio_data)); % Number of samples to use
audio_data = audio_data(1:num_samples);
t = (0:num_samples-1) / fs; % Time vector in seconds
...
received_signals = zeros(N, num_samples);
for i = 1:N
